  ...
  document.body.insertAdjacentHTML('beforeend', `
    <style>
      ...
      *{box-sizing:border-box}html,body{margin:0;min-height:100%;font-family:"SF Pro Display","Segoe UI","Microsoft YaHei",sans-serif;color:var(--ink);letter-spacing:0}
      ...
      .scene::before{content:"";position:absolute;inset:0;z-index:-2;background:var(--bg-dim-color, linear-gradient(90deg,rgba(14,37,48,.14),rgba(236,240,237,.02) 52%,rgba(241,222,213,.06)));transition:background .2s ease}
      ...
      .glass{position:relative;overflow:hidden;border:1px solid var(--line);background:var(--glass-bg, linear-gradient(145deg,rgba(255,255,255,.36),rgba(225,241,247,.14)));box-shadow:0 20px 55px rgba(14,45,62,.17),inset 0 1px 0 rgba(255,255,255,.85),inset 0 -1px 0 rgba(255,255,255,.14);backdrop-filter:blur(var(--glass-blur, 22px)) saturate(155%);-webkit-backdrop-filter:blur(var(--glass-blur, 22px)) saturate(155%);transition:backdrop-filter .2s ease,background .2s ease}
      ...
      .input-shell:focus-within{border-color:var(--accent);box-shadow:0 0 0 4px var(--accent-glow, rgba(23,147,153,.12)),inset 0 1px 0 white;background:rgba(255,255,255,.56)}
      input[type=text],input[type=password]{width:100%;height:100%;border:0!important;border-radius:0!important;background:transparent!important;padding:0 15px;color:var(--ink);font:15px inherit;outline:0!important;box-shadow:none!important;-webkit-appearance:none;appearance:none}
      .captcha-row{display:grid;grid-template-columns:1fr 112px;gap:9px}.captcha-image{width:112px;height:46px;border-radius:14px;border:1px solid rgba(255,255,255,.68);object-fit:fill;background:rgba(255,255,255,.3);box-shadow:inset 0 1px white;cursor:pointer}.hint{margin-top:6px;color:var(--muted);font-size:11px}.remember{position:relative;display:flex;align-items:center;gap:8px;margin:17px 0;color:#405b6c;font-size:13px;cursor:pointer}.remember input{width:16px;height:16px;accent-color:var(--accent)}.submit{position:relative;width:100%;height:48px;border:1px solid rgba(255,255,255,.62);border-radius:15px;color:white;background:var(--accent-gradient, linear-gradient(135deg,rgba(6,139,146,.88),rgba(32,108,154,.8)));box-shadow:0 11px 26px var(--accent-glow, rgba(13,105,128,.21)),inset 0 1px rgba(255,255,255,.44);font:700 14px inherit;cursor:pointer;transition:transform .2s,filter .2s}
      ...
      .login-bottom-actions{position:absolute;right:24px;bottom:24px;z-index:3;display:flex;align-items:center;gap:10px}
      .action-btn,.background-customize{height:46px;padding:0 17px;border-radius:15px;color:white;border:1px solid rgba(255,255,255,.62);background:linear-gradient(145deg,rgba(11,39,52,.42),rgba(225,241,247,.18));box-shadow:0 16px 38px rgba(14,45,62,.16),inset 0 1px rgba(255,255,255,.72);backdrop-filter:blur(var(--glass-blur, 22px)) saturate(155%);-webkit-backdrop-filter:blur(var(--glass-blur, 22px)) saturate(155%);font:650 13px inherit;cursor:pointer;display:inline-flex;align-items:center;gap:9px;transition:transform .18s,background .18s,box-shadow .18s}
      .action-btn:hover,.background-customize:hover{background:linear-gradient(145deg,rgba(11,39,52,.54),rgba(225,241,247,.28));transform:translateY(-1px);box-shadow:0 18px 42px rgba(14,45,62,.22),inset 0 1px white}
      .picture-icon{position:relative;width:18px;height:15px;border:1.7px solid currentColor;border-radius:3px}.picture-icon::before{content:"";position:absolute;width:4px;height:4px;right:2px;top:2px;border-radius:50%;background:currentColor}.picture-icon::after{content:"";position:absolute;left:2px;right:2px;bottom:2px;height:7px;background:linear-gradient(135deg,transparent 36%,currentColor 37% 52%,transparent 53%),linear-gradient(45deg,transparent 44%,currentColor 45% 61%,transparent 62%)}
      .background-editor{position:fixed;inset:0;z-index:100;display:none;align-items:center;justify-content:center;padding:24px;background:rgba(10,25,34,.45);backdrop-filter:blur(14px) saturate(125%);-webkit-backdrop-filter:blur(14px) saturate(125%)}.background-editor.open{display:flex}.crop-dialog{width:min(940px,calc(100vw - 48px));max-height:calc(100vh - 48px);overflow:auto;padding:22px;border-radius:22px;color:var(--ink);background:linear-gradient(145deg,rgba(245,250,252,.74),rgba(218,235,242,.48));display:flex;flex-direction:column;gap:16px}.crop-header{position:relative;display:flex;align-items:start;justify-content:space-between;gap:18px}.crop-header h2{margin:0;font-size:22px}.crop-header p{margin:5px 0 0;color:var(--muted);font-size:12px}.crop-close{width:38px;height:38px;flex:none;border:1px solid rgba(255,255,255,.75);border-radius:12px;background:rgba(255,255,255,.35);color:#27475a;font:25px/1 inherit;cursor:pointer}.crop-stage{position:relative;flex:none;align-self:center;overflow:hidden;border-radius:15px;background:#17232a;box-shadow:inset 0 0 0 1px rgba(255,255,255,.4);cursor:grab;touch-action:none}.crop-stage.dragging{cursor:grabbing}.crop-stage img{position:absolute;max-width:none;user-select:none;pointer-events:none}.crop-stage::after{content:"";position:absolute;inset:0;pointer-events:none;border:1px solid rgba(255,255,255,.74);border-radius:15px;box-shadow:inset 0 0 40px rgba(3,17,23,.12)}.crop-controls{position:relative;display:flex;align-items:center;gap:12px}.crop-controls label{font-size:13px;font-weight:650}.crop-controls input{flex:1;accent-color:var(--accent)}.crop-actions{position:relative;display:flex;align-items:center;justify-content:space-between;gap:10px}.crop-actions-left,.crop-actions-right{display:flex;gap:9px}.crop-button{height:42px;padding:0 16px;border-radius:13px;border:1px solid rgba(255,255,255,.7);background:rgba(255,255,255,.34);color:#294758;font:650 13px inherit;cursor:pointer}.crop-button.primary{min-width:108px;color:white;background:var(--accent-gradient, linear-gradient(135deg,rgba(6,139,146,.92),rgba(32,108,154,.88)));box-shadow:0 6px 18px var(--accent-glow)}.crop-button:disabled{opacity:.62;cursor:wait}.crop-message{position:relative;min-height:17px;color:#a43b35;font-size:12px;text-align:center}
      @media(max-height:760px){.copy{display:none}.identity{padding:16px 20px}}
      @media(max-width:820px){body{overflow-y:auto}.scene{min-height:100vh;height:auto;overflow-y:auto;-webkit-overflow-scrolling:touch;background-position:center center;padding:16px 14px 90px}.left-rail{position:relative;top:auto;bottom:auto;left:auto;width:100%;max-width:400px;margin:0 auto;gap:14px}.identity{padding:14px 18px}.brand{font-size:16px}.brand img{width:36px;height:36px}.copy{display:none}.login-panel{padding:20px}.login-bottom-actions{position:fixed;right:14px;bottom:14px;z-index:99;gap:8px}.action-btn,.background-customize{height:38px;padding:0 12px;font-size:12px}.crop-dialog{width:calc(100vw - 28px);padding:16px}.crop-actions{align-items:stretch;flex-direction:column}.crop-actions-left,.crop-actions-right{display:grid;grid-template-columns:1fr 1fr}.crop-button{padding:0 11px}}
    </style>
    <main class="scene"><video id="scene-video" class="scene-video" style="display:none;" autoplay muted loop playsinline></video><div class="left-rail">
    ...
    </div><div class="login-bottom-actions"><button class="action-btn background-customize" id="background-customize" type="button" title="选择并裁切自定义背景"><span class="picture-icon" aria-hidden="true"></span><span>自定义背景</span></button><button class="action-btn" id="ui-customize-btn" type="button" title="自定义界面风格 (Ctrl+U / F4)"><span style="font-size:16px;" aria-hidden="true">🎨</span><span>自定义 UI</span></button></div></main>
